...
#define abort_on(str)                                                          \
  { B_THROW1(ParseException, std::string("abort_on ") + str); }
...
int unhex(char c) {
  ...
  B_THROW1(wasm::ParseException, "invalid hexadecimal");
}
} // namespace
...
checkAddress(Address a, const char* errorText, const Element* errorElem) {
  if (a > std::numeric_limits<Address::address32_t>::max()) {
    B_THROW_DUMP(ParseException(errorText, errorElem->line, errorElem->col));
  }
}
...
Element::List& Element::list() {
  if (!isList()) {
    B_THROW_DUMP(ParseException("expected list", line, col));
  }
  return list_;
}
...
Element* Element::operator[](unsigned i) {
  ...
  if (i >= list().size()) {
    B_THROW_DUMP(ParseException("expected more elements in list", line, col));
  }
  return list()[i];
}
...
IString Element::str() const {
  if (!isStr()) {
    B_THROW_DUMP(ParseException("expected string", line, col));
  }
  return str_;
}
...
Element* SExpressionParser::parse() {
  ...
  while (1) {
    ...
    if (input[0] == '(') {
      ...
      auto last = curr;
      if (stack.empty()) {
        B_THROW_DUMP(ParseException("s-expr stack empty"));
      }
      curr = stack.back();
      assert(stack.size() == stackLocs.size());
      stack.pop_back();
  ...
  if (stack.size() != 0) {
    B_THROW_DUMP(ParseException("stack is not empty", curr->line, curr->col));
  }
  return curr;
}
...
Element* SExpressionParser::parseString() {
  ...
  if (input[0] == '"') {
    ...
    while (1) {
      if (input[0] == 0) {
        B_THROW_DUMP(ParseException("unterminated string", line, start - lineStart));
      }
      if (input[0] == '"') {
        break;
      }
  ...
  if (start == input) {
    B_THROW_DUMP(ParseException("expected string", line, input - lineStart));
  }
...
SExpressionWasmBuilder::SExpressionWasmBuilder(Module& wasm,
  ...
  if (module.size() == 0) {
    B_THROW_DUMP(ParseException("empty toplevel, expected module"));
  }
  if (module[0]->str() != MODULE) {
    B_THROW_DUMP(ParseException("toplevel does not start with module"));
  }
  if (module.size() == 1) {
    return;
  }
...
void SExpressionWasmBuilder::parseModuleElement(Element& curr) {
  ...
  B_THROW_DUMP(ParseException("unknown module element", curr.line, curr.col));
}
...
Name SExpressionWasmBuilder::getTableName(Element& s) {
  if (s.dollared()) {
    ...
    if (offset >= tableNames.size()) {
      B_THROW_DUMP(ParseException("unknown table in getTableName", s.line, s.col));
    }
...
bool SExpressionWasmBuilder::isMemory64(Name memoryName) {
  ...
  if (!memory) {
    B_THROW_DUMP(ParseException("invalid memory name in isMemory64"));
  }
  return memory->is64();
}
...
Name SExpressionWasmBuilder::getMemoryNameAtIdx(Index i) {
  if (i >= memoryNames.size()) {
    B_THROW_DUMP(ParseException("unknown memory in getMemoryName"));
  }
...
Name SExpressionWasmBuilder::getGlobalName(Element& s) {
  if (s.dollared()) {
    ...
    if (offset >= globalNames.size()) {
      B_THROW_DUMP(ParseException("unknown global in getGlobalName", s.line, s.col));
    }
...
Name SExpressionWasmBuilder::getTagName(Element& s) {
  if (s.dollared()) {
    ...
    if (offset >= tagNames.size()) {
      B_THROW_DUMP(ParseException("unknown tag in getTagName", s.line, s.col));
    }
...
SExpressionWasmBuilder::parseParamOrLocal(Element& s, size_t& localIndex) {
  ...
  for (size_t i = 1; i < s.size(); i++) {
    ...
    if (s[i]->dollared()) {
      if (i != 1) {
        B_THROW_DUMP(ParseException("invalid wasm type", s[i]->line, s[i]->col));
      }
      if (i + 1 >= s.size()) {
        B_THROW_DUMP(ParseException("invalid param entry", s.line, s.col));
      }
      name = s[i]->str();
      i++;
    } else {
...
HeapType SExpressionWasmBuilder::parseTypeRef(Element& s) {
  ...
  if (s.size() != 2) {
    B_THROW_DUMP(ParseException("invalid type reference", s.line, s.col));
  }
  auto heapType = parseHeapType(*s[1]);
  if (!heapType.isSignature()) {
    B_THROW_DUMP(ParseException("expected signature type", s.line, s.col));
  }
  return heapType;
}
...
SExpressionWasmBuilder::parseTypeUse(Element& s,
  ...
  if (!typeExists) {
    ...
    if (inlineSig != functionType.getSignature()) {
      B_THROW_DUMP(ParseException("type and param/result don't match",
                           s[paramPos]->line,
                           s[paramPos]->col));
    }
  }
...
void SExpressionWasmBuilder::preParseHeapTypes(Element& module) {
  ...
  forEachType([&](Element& elem, size_t) {
    if (elem[1]->dollared()) {
      ...
      if (!typeIndices.insert({name, numTypes}).second) {
        B_THROW_DUMP(ParseException("duplicate function type", elem.line, elem.col));
      }
    }
    ++numTypes;
  });
  ...
  auto parseRefType = [&](Element& elem) -> Type {
    ...
    if (referent.dollared()) {
      ...
      if (index >= numTypes) {
        B_THROW_DUMP(ParseException("invalid type index", elem.line, elem.col));
      }
  ...
  auto parseValType = [&](Element& elem) {
    if (elem.isStr()) {
      ...
      B_THROW_DUMP(ParseException("unknown valtype kind", elem[0]->line, elem[0]->col));
    }
  };
  ...
  forEachType([&](Element& elem, size_t) {
    ...
    if (kind == FUNC || kind == FUNC_SUBTYPE) {
      ...
      B_THROW_DUMP(ParseException("unknown heaptype kind", kind.line, kind.col));
    }
    ...
    if (hasSupertype) {
      ...
      if (super->dollared()) {
        ...
        B_THROW_DUMP(ParseException("unknown supertype", super->line, super->col));
      }
...
void SExpressionWasmBuilder::parseFunction(Element& s, bool preParseImport) {
  ...
  if (exportName.is()) {
    ...
    if (wasm.getExportOrNull(ex->name)) {
      B_THROW_DUMP(ParseException("duplicate export", s.line, s.col));
    }
  ...
  if (importModule.is()) {
    if (!importBase.size()) {
      B_THROW_DUMP(ParseException("module but no base for import", s.line, s.col));
    }
    if (!preParseImport) {
      B_THROW_DUMP(ParseException("!preParseImport in func", s.line, s.col));
    }
    ...
    if (wasm.getFunctionOrNull(im->name)) {
      B_THROW_DUMP(ParseException("duplicate import", s.line, s.col));
    }
    wasm.addFunction(std::move(im));
    if (currFunction) {
      B_THROW_DUMP(ParseException("import module inside function dec", s.line, s.col));
    }
    nameMapper.clear();
    return;
  }
  ...
  if (wasm.getFunctionOrNull(currFunction->name)) {
    B_THROW_DUMP(ParseException("duplicate function", s.line, s.col));
  }
...
Type SExpressionWasmBuilder::stringToType(std::string_view str,
  ...
  B_THROW_DUMP(ParseException(std::string("invalid wasm type: ") +
...
HeapType SExpressionWasmBuilder::stringToHeapType(std::string_view str,
  ...
  B_THROW_DUMP(ParseException(std::string("invalid wasm heap type: ") +
                       std::string(str.data(), str.size())));
}
...
Index SExpressionWasmBuilder::getLocalIndex(Element& s) {
  if (!currFunction) {
    B_THROW_DUMP(ParseException("local access in non-function scope", s.line, s.col));
  }
  if (s.dollared()) {
    auto ret = s.str();
    if (currFunction->localIndices.count(ret) == 0) {
      B_THROW_DUMP(ParseException("bad local name", s.line, s.col));
    }
  ...
  if (ret >= currFunction->getNumLocals()) {
    B_THROW_DUMP(ParseException("bad local index", s.line, s.col));
  }
  return ret;
}
...
Expression* SExpressionWasmBuilder::makeGlobalGet(Element& s) {
  ...
  if (!global) {
    B_THROW_DUMP(ParseException("bad global.get name", s.line, s.col));
  }
...
Expression* SExpressionWasmBuilder::makeGlobalSet(Element& s) {
  ...
  if (wasm.getGlobalOrNull(ret->name) &&
    ...
    B_THROW_DUMP(ParseException("global.set of immutable", s.line, s.col));
  }
  ret->value = parseExpression(s[2]);
  ret->finalize();
  return ret;
...
static Expression* parseConst(IString s, Type type, MixedArena& allocator) {
  ...
  if (type.isFloat()) {
    ...
    if (positive[0] == 'n' && positive[1] == 'a' && positive[2] == 'n') {
      ...
      if (!(modifier ? positive[4] == '0' && positive[5] == 'x' : 1)) {
        B_THROW_DUMP(ParseException("bad nan input"));
      }
      switch (type.getBasic()) {
        case Type::f32: {
          uint32_t pattern;
          if (modifier) {
            ...
            if (istr.fail()) {
              B_THROW_DUMP(ParseException("invalid f32 format"));
            }
            pattern |= 0x7f800000U;
          } else {
            pattern = 0x7fc00000U;
        ...
        case Type::f64: {
          ...
          if (modifier) {
            ...
            istr >> std::hex >> pattern;
            if (istr.fail()) {
              B_THROW_DUMP(ParseException("invalid f64 format"));
            }
  ...
  switch (type.getBasic()) {
    case Type::i32: {
      if ((str[0] == '0' && str[1] == 'x') ||
        ...
        istr >> std::hex >> temp;
        if (istr.fail()) {
          B_THROW_DUMP(ParseException("invalid i32 format"));
        }
    ...
    case Type::i64: {
      if ((str[0] == '0' && str[1] == 'x') ||
        ...
        istr >> std::hex >> temp;
        if (istr.fail()) {
          B_THROW_DUMP(ParseException("invalid i64 format"));
        }
        ret->value = Literal(negative ? -temp : temp);
      } else {
        std::istringstream istr(str[0] == '-' ? str + 1 : str);
  ...
  if (ret->value.type != type) {
    B_THROW_DUMP(ParseException("parsed type does not match expected type"));
  }
  return ret;
}
...
Expression* SExpressionWasmBuilder::makeConst(Element& s, Type type) {
  if (type != Type::v128) {
    ...
    if (!ret) {
      B_THROW_DUMP(ParseException("bad const", s[1]->line, s[1]->col));
    }
    return ret;
  }
  ...
  while (!s[i]->isList()) {
    ...
    if (str[0] == 'a') {
      if (value > std::numeric_limits<uint32_t>::max()) {
        B_THROW_DUMP(ParseException("bad align", s[i]->line, s[i]->col));
      }
      align = value;
    } else if (str[0] == 'o') {
      if (!memory64 && value > std::numeric_limits<uint32_t>::max()) {
        B_THROW_DUMP(ParseException("bad offset", s[i]->line, s[i]->col));
      }
      offset = value;
    } else {
      B_THROW_DUMP(ParseException("bad memory attribute", s[i]->line, s[i]->col));
    }
    i++;
  }
  return i;
...
Expression* SExpressionWasmBuilder::makeAtomicRMW(Element& s,
  ...
  if (align != ret->bytes) {
    B_THROW_DUMP(ParseException("Align of Atomic RMW must match size", s.line, s.col));
  }
  ret->ptr = parseExpression(s[i]);
  ret->value = parseExpression(s[i + 1]);
  ret->finalize();
...
static uint8_t parseLaneIndex(const Element* s, size_t lanes) {
  ...
  if (end == str || *end != '\0') {
    B_THROW_DUMP(ParseException("Expected lane index", s->line, s->col));
  }
  if (n > lanes) {
    B_THROW_DUMP(ParseException(
...
Expression* SExpressionWasmBuilder::makeCallIndirect(Element& s,
  ...
  if (wasm.tables.empty()) {
    B_THROW_DUMP(ParseException("no tables", s.line, s.col));
  }
  Index i = 1;
...
Expression* SExpressionWasmBuilder::makeBreakTable(Element& s) {
  ...
  if (ret->targets.size() == 0) {
    B_THROW_DUMP(ParseException("switch with no targets", s.line, s.col));
  }
...
Expression* SExpressionWasmBuilder::makeRefNull(Element& s) {
  if (s.size() != 2) {
    B_THROW_DUMP(ParseException("invalid heap type reference", s.line, s.col));
  }
...
Expression* SExpressionWasmBuilder::makeTableGet(Element& s) {
  ...
  if (!table) {
    B_THROW_DUMP(ParseException("invalid table name in table.get", s.line, s.col));
  }
...
Expression* SExpressionWasmBuilder::makeTableSet(Element& s) {
  ...
  auto* table = wasm.getTableOrNull(tableName);
  if (!table) {
    B_THROW_DUMP(ParseException("invalid table name in table.set", s.line, s.col));
  }
  auto* index = parseExpression(s[2]);
...
Expression* SExpressionWasmBuilder::makeTableSize(Element& s) {
  ...
  auto* table = wasm.getTableOrNull(tableName);
  if (!table) {
    B_THROW_DUMP(ParseException("invalid table name in table.size", s.line, s.col));
  }
...
Expression* SExpressionWasmBuilder::makeTableGrow(Element& s) {
  ...
  auto* table = wasm.getTableOrNull(tableName);
  if (!table) {
    B_THROW_DUMP(ParseException("invalid table name in table.grow", s.line, s.col));
  }
  auto* value = parseExpression(s[2]);
  if (!value->type.isRef()) {
    B_THROW_DUMP(ParseException("only reference types are valid for tables"));
  }
...
Expression* SExpressionWasmBuilder::makeTry(Element& s) {
  ...
  while (i < s.size() && elementStartsWith(*s[i], "catch")) {
    Element& inner = *s[i++];
    if (inner.size() < 2) {
      B_THROW_DUMP(ParseException("invalid catch block", inner.line, inner.col));
    }
    Name tag = getTagName(*inner[1]);
    if (!wasm.getTagOrNull(tag)) {
      B_THROW_DUMP(ParseException("bad tag name", inner[1]->line, inner[1]->col));
    }
  ...
  if (i < s.size() && elementStartsWith(*s[i], "delegate")) {
    Element& inner = *s[i++];
    if (inner.size() != 2) {
      B_THROW_DUMP(ParseException("invalid delegate", inner.line, inner.col));
    }
...
Expression* SExpressionWasmBuilder::makeThrow(Element& s) {
  ...
  if (!wasm.getTagOrNull(ret->tag)) {
    B_THROW_DUMP(ParseException("bad tag name", s[1]->line, s[1]->col));
  }
  for (; i < s.size(); i++) {
...
Expression* SExpressionWasmBuilder::makeTupleExtract(Element& s) {
  ...
  if (ret->tuple->type != Type::unreachable &&
    ...
    B_THROW_DUMP(ParseException("Bad index on tuple.extract", s[1]->line, s[1]->col));
  }
  ret->finalize();
  return ret;
}
...
Expression* SExpressionWasmBuilder::makeStructNew(Element& s, bool default_) {
  ...
  if (default_ && numOperands > 0) {
    B_THROW_DUMP(ParseException("arguments provided for struct.new", s.line, s.col));
  }
  std::vector<Expression*> operands;
...
Index SExpressionWasmBuilder::getStructIndex(Element& type, Element& field) {
  if (field.dollared()) {
    ...
    B_THROW_DUMP(ParseException("bad struct field name", field.line, field.col));
  }
  // this is a numeric index
...
Expression* SExpressionWasmBuilder::makeStructGet(Element& s, bool signed_) {
  ...
  if (!heapType.isStruct()) {
    B_THROW_DUMP(ParseException("bad struct heap type", s.line, s.col));
  }
...
Expression* SExpressionWasmBuilder::makeArrayGet(Element& s, bool signed_) {
  auto heapType = parseHeapType(*s[1]);
  if (!heapType.isArray()) {
    B_THROW_DUMP(ParseException("bad array heap type", s.line, s.col));
  }
  auto ref = parseExpression(*s[2]);
...
Expression* SExpressionWasmBuilder::makeRefAs(Element& s, RefAsOp op) {
  ...
  if (!value->type.isRef() && value->type != Type::unreachable) {
    B_THROW_DUMP(ParseException("ref.as child must be a ref", s.line, s.col));
  }
  ...
  if (op == StringNewWTF8 || op == StringNewUTF8) {
    if (!try_) {
      ...
      if (str == "utf8") {
        ...
        B_THROW_DUMP(ParseException("bad string.new op", s.line, s.col));
      }
    }
...
Expression* SExpressionWasmBuilder::makeStringMeasure(Element& s,
  ...
  if (op == StringMeasureWTF8) {
    ...
    if (str == "utf8") {
      ...
      B_THROW_DUMP(ParseException("bad string.measure op", s.line, s.col));
    }
  }
...
void SExpressionWasmBuilder::stringToBinary(Element& s,
  ...
  for (const char* input = str.data(); input < end;) {
    if (input[0] == '\\') {
      if (input + 1 >= end) {
        B_THROW_DUMP(ParseException("Unterminated escape sequence", s.line, s.col));
      }
      if (input[1] == 't') {
        *write++ = '\t';
        input += 2;
...
Index SExpressionWasmBuilder::parseMemoryLimits(
  ...
  if (i == s.size()) {
    B_THROW_DUMP(ParseException("missing memory limits", s.line, s.col));
  }
  auto initElem = s[i++];
...
void SExpressionWasmBuilder::parseMemory(Element& s, bool preParseImport) {
  ...
  if (s[i]->isList()) {
    ...
    if (elementStartsWith(inner, EXPORT)) {
      ...
      if (wasm.getExportOrNull(ex->name)) {
        B_THROW_DUMP(ParseException("duplicate export", inner.line, inner.col));
      }
      wasm.addExport(ex.release());
      i++;
      ...
      if (!(inner.size() > 0 ? inner[0]->str() != IMPORT : true)) {
        B_THROW_DUMP(ParseException("bad import ending", inner.line, inner.col));
      }
      // (memory (data ..)) format
...
void SExpressionWasmBuilder::parseExport(Element& s) {
  ...
  if (s[2]->isList()) {
    ...
    if (elementStartsWith(inner, FUNC)) {
      ...
      B_THROW_DUMP(ParseException("invalid export", inner.line, inner.col));
    }
  } else {
    // function
    ex->value = s[2]->str();
...
void SExpressionWasmBuilder::parseImport(Element& s) {
  ...
  if (!hasExplicitName) {
    if (kind == ExternalKind::Function) {
      ...
      B_THROW_DUMP(ParseException("invalid import", s[3]->line, s[3]->col));
    }
  }
  if (!newStyle) {
    kind = ExternalKind::Function;
  ...
  auto module = s[i++]->str();
  if (!s[i]->isStr()) {
    B_THROW_DUMP(ParseException("no name for import", s[i]->line, s[i]->col));
  }
  auto base = s[i]->str();
  ...
  if (j < inner.size()) {
    B_THROW_DUMP(ParseException("invalid element", inner[j]->line, inner[j]->col));
  }
}
...
void SExpressionWasmBuilder::parseGlobal(Element& s, bool preParseImport) {
  ...
  if (exported && mutable_) {
    B_THROW_DUMP(ParseException("cannot export a mutable global", s.line, s.col));
  }
  if (type == Type::none) {
  ...
  if (importModule.is()) {
    ...
    if (!preParseImport) {
      B_THROW_DUMP(ParseException("!preParseImport in global", s.line, s.col));
    }
  ...
  if (i < s.size()) {
    ...
    B_THROW_DUMP(ParseException("global without init", s.line, s.col));
  }
  global->mutable_ = mutable_;
  if (i != s.size()) {
    B_THROW_DUMP(ParseException("extra import elements", s.line, s.col));
  }
...
void SExpressionWasmBuilder::parseTable(Element& s, bool preParseImport) {
  ...
  if (s[i]->isList()) {
    ...
    if (elementStartsWith(inner, EXPORT)) {
      ...
      if (!preParseImport) {
        B_THROW_DUMP(ParseException("!preParseImport in table", inner.line, inner.col));
      }
      table->module = inner[1]->str();
      table->base = inner[2]->str();
      i++;
    } else if (!elementStartsWith(inner, REF)) {
      B_THROW_DUMP(ParseException("invalid table", inner.line, inner.col));
    }
  }
...
void SExpressionWasmBuilder::parseElem(Element& s, Table* table) {
  ...
  if (!isPassive && segment->table.isNull()) {
    if (wasm.tables.empty()) {
      B_THROW_DUMP(ParseException("active element without table", s.line, s.col));
    }
...
ElementSegment* SExpressionWasmBuilder::parseElemFinish(
  ...
  for (; i < s.size(); i++) {
    ...
    if (!usesExpressions) {
      B_THROW_DUMP(ParseException("expected an MVP-style $funcname in elem."));
    }
    auto& inner = *s[i];
...
HeapType SExpressionWasmBuilder::parseHeapType(Element& s) {
  if (s.isStr()) {
    ...
    if (s.dollared()) {
      ...
      if (it == typeIndices.end()) {
        B_THROW_DUMP(ParseException("unknown dollared function type", s.line, s.col));
      }
      ...
      if (String::isNumber(str)) {
        ...
        if (offset >= types.size()) {
          B_THROW_DUMP(ParseException("unknown indexed function type", s.line, s.col));
        }
        return types[offset];
      }
  ...
  B_THROW_DUMP(ParseException("invalid heap type", s.line, s.col));
}
...
void SExpressionWasmBuilder::parseTag(Element& s, bool preParseImport) {
  ...
  if (s[i]->isStr() && s[i]->dollared()) {
    ...
    if (wasm.getTagOrNull(tag->name)) {
      B_THROW_DUMP(ParseException("duplicate tag", inner.line, inner.col));
    }
  } else {
  ...
  if (i < s.size() && elementStartsWith(*s[i], IMPORT)) {
    ...
    if (importElem.size() != 3) {
      B_THROW_DUMP(ParseException("invalid import", importElem.line, importElem.col));
    }
  ...
  if (i < s.size() && elementStartsWith(*s[i], EXPORT)) {
    auto& exportElem = *s[i++];
    if (tag->module.is()) {
      B_THROW_DUMP(ParseException("import and export cannot be specified together",
                           exportElem.line,
                           exportElem.col));
    }
    if (exportElem.size() != 2) {
      B_THROW_DUMP(ParseException("invalid export", exportElem.line, exportElem.col));
    }
  ...
  // If there are more elements, they are invalid
  if (i < s.size()) {
    B_THROW_DUMP(ParseException("invalid element", s[i]->line, s[i]->col));
  }
...
void SExpressionWasmBuilder::validateHeapTypeUsingChild(Expression* child,
  ...
  if (!child->type.isRef() ||
    ...
    B_THROW_DUMP(ParseException("bad heap type: expected " + heapType.toString() +
